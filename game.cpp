...
void Game::detectVersion() {
    ...
    static const char *startupScenesTable[] = { "SCN\\-01.SCN", "SCN\\_01.SCN", 0 };
    _startupScene = 0;
...
void Game::updateKeysPressedTable() {
    ...
    if (_stub->_pi.load) {
        ...
        char filePath[MAX_FILE_PATH_LENGTH];
        snprintf(filePath, MAX_FILE_PATH_LENGTH, kGameStateFileNameFormat, _savePath, _stateSlot);
        File f;
        if (!f.open(filePath, "rb")) {
            warning("Unable to load game state from file '%s'", filePath);
        } else {
...
void Game::playMusic(const char *name) {
    ...
    for (unsigned int i = 0; i < ARRAYSIZE(_midiMapping); ++i) {
        if (strcasecmp(_midiMapping[i].fileName, name + 8) == 0) {
            char filePath[MAX_FILE_PATH_LENGTH];
            snprintf(filePath, MAX_FILE_PATH_LENGTH, "%s/track%02d.ogg", _musicPath, _midiMapping[i].digitalTrack);
